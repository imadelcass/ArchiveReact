...
const PiecesManquants = () => {
  ...
  const handleDossPrint = useReactToPrint({
    ...
    // pageStyle:
    //   "@page { size: A4 ;}.bg-gray-300{background: blueviolet !important}",
    // copyStyles: true,
  });
    ...
    <div className="w-full">
      {!printPieces && (
        <div ref={dossCompoRef} className={`w-full table $style.page}`}>
          ...
          {!printDoss && <Grid columns={Columns()} setDossier={setDossier} />}
          {printDoss && (
            <div
...
where idTypeDossier in
(select idTypeDossier from composit_dossiers
where idTypePiece not in (select idTypePiece from pieces ))
